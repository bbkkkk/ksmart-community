...
import com.ksmart.pms.biz.dao.RoleDao;
import com.ksmart.pms.biz.domain.Role;
import com.ksmart.pms.biz.service.IRoleService;
...
public class RoleService implements IRoleService {
    ...
    public int delete(String id) {
        roleDao.delete(new Role().setId(id));
        return 1;
    }

    @Override
    public RoleDTO get(String id) {
        Optional<Role> optional = roleDao.findOne(Example.of(new Role().setId(id)));
        RoleDTO roleDTO = new RoleDTO();
        optional.ifPresent(role -> BeanUtils.copyProperties(role, roleDTO));
        log.debug("返回结果 {}", roleDTO);
        return roleDTO;
    }

    @Override
    public List<RoleDTO> query(String input) {
        List<Role> roleList = roleDao.findRoleByNameLike(input);
    ...
    @Override
    public PageData<Role> queryPage(PageDTO pageDTO) {
        ...
        // 指定排序和分页
        Page<Role> page = roleDao.findAll(specification, pageRequest);
        ...
        return PageData.<Role>getInstance(currentPage, pageSize, page.getTotalElements(), page.getContent());
    }
