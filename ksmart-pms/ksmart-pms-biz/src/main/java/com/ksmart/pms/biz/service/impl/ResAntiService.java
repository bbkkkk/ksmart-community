...
import com.ksmart.pms.biz.dao.ResAntiDao;
import com.ksmart.pms.biz.domain.ResAnti;
import com.ksmart.pms.biz.service.IResAntiService;
...
@Log4j2
@Service
public class ResAntiService implements IResAntiService {

    @Autowired
    private ResAntiDao resAntiDao;


    @Override
    public int insert(ResAntiDTO resAntiDTO) {
        ResAnti resAnti = new ResAnti();
        BeanUtils.copyProperties(resAntiDTO, resAnti);
        resAntiDao.save(resAnti);
        return 1;
    }

    @Override
    public int update(ResAntiDTO resAntiDTO) {
        ResAnti resAnti = new ResAnti();
        BeanUtils.copyProperties(resAntiDTO, resAnti);
        resAntiDao.save(resAnti);
        return 1;
    ...
    public int delete(String id) {
        resAntiDao.delete(new ResAnti().setId(id));
        return 1;
    }

    @Override
    public ResAntiDTO get(String id) {
        Optional<ResAnti> optional = resAntiDao.findOne(Example.of(new ResAnti().setId(id)));
        ResAntiDTO resAntiDTO = new ResAntiDTO();
        optional.ifPresent(resAnti -> BeanUtils.copyProperties(resAnti, resAntiDTO));
        log.debug("返回结果 {}", resAntiDTO);
        return resAntiDTO;
    }

    @Override
    public List<ResAntiDTO> query(String input) {
        List<ResAnti> resAntiList = resAntiDao.findResAntiByResLnameLike(input);
        ...
        return resAntiDTOList;
    }

    @Override
    public PageData<ResAnti> queryPage(PageDTO pageDTO) {
        ...
        Specification<ResAnti> specification = (Specification<ResAnti>) (root, criteriaQuery, criteriaBuilder) -> {
            // 查询条件的集合
        ...
        // 指定排序和分页
        Page<ResAnti> page = resAntiDao.findAll(specification, pageRequest);
        ...
        return PageData.<ResAnti>getInstance(currentPage, pageSize, page.getTotalElements(), page.getContent());
    }
