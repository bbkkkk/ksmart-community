...
import com.ksmart.pms.biz.dao.RoleResDao;
import com.ksmart.pms.biz.domain.RoleRes;
import com.ksmart.pms.biz.service.IRoleResService;
...
public class RoleResService implements IRoleResService {
    ...
    @Override
    public int insert(RoleResDTO roleResDTO) {
        RoleRes roleRes = new RoleRes();
        BeanUtils.copyProperties(roleResDTO, roleRes);
        roleResDao.save(roleRes);
        return 1;
    }

    @Override
    public int update(RoleResDTO roleResDTO) {
        RoleRes roleRes = new RoleRes();
        BeanUtils.copyProperties(roleResDTO, roleRes);
        roleResDao.save(roleRes);
        return 1;
    ...
    public int delete(String id) {
        roleResDao.delete(new RoleRes().setId(id));
        return 1;
    }

    @Override
    public RoleResDTO get(String id) {
        Optional<RoleRes> optional = roleResDao.findOne(Example.of(new RoleRes().setId(id)));
        RoleResDTO roleResDTO = new RoleResDTO();
        optional.ifPresent(roleRes -> BeanUtils.copyProperties(roleRes, roleResDTO));
        log.debug("返回结果 {}", roleResDTO);
        return roleResDTO;
    }

    @Override
    public List<RoleResDTO> query(String input) {
        List<RoleRes> roleResList = roleResDao.findRoleResByRoleNameLike(input);
    ...
    @Override
    public PageData<RoleRes> queryPage(PageDTO pageDTO) {
        ...
        Specification<RoleRes> specification = (Specification<RoleRes>) (root, criteriaQuery, criteriaBuilder) -> {
            // 查询条件的集合
        ...
        // 指定排序和分页
        Page<RoleRes> page = roleResDao.findAll(specification, pageRequest);
        ...
        return PageData.<RoleRes>getInstance(currentPage, pageSize, page.getTotalElements(), page.getContent());
    }
